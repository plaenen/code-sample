...
/**
 * calculate the reduction for a given segment based on an amount summary and segmentation reduction rules
...
module.exports = (userProfile, amounts, segmentReductionRules) => {
  ...
  // Don't apply reductions on the total amount, take the grand total and reduce previous reductions 
  ...
  if (loyaltyInYears >= reductionRules.minLoyaltyInYears) {
    ...
    result.description = "Applied an user segment reduction (" + userProfile.userSegment + ") of " + reductionRules.percentage + "% on the amount of $" + baseAmount;
  }
